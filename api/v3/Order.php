    ...
    foreach ($params['line_items'] as $lineItems) {
      ...
      if ($entityParams) {
        if (in_array($entity, ['participant', 'membership'])) {
          ...
          $entityParams['status_id'] = ($entity === 'participant' ? $entityParams['participant_status_id'] ?? 'Pending from incomplete transaction' : 'Pending');
      ...
      if (empty($priceSetID) && !empty($lineItems['line_item'])) {
        $item = reset($lineItems['line_item']);
